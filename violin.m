...
for i=1:size(Y,2)
    ...
    if isempty(b)==0 
        [f, u, bb]=ksdensity(Y{i},'bandwidth',b(i),'support',[0 1]);
    elseif isempty(b)
        [f, u, bb]=ksdensity(Y{i},'support',[0 15]);
    end
...
for i=i:size(Y,2)
    ...
    if setX == 0
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i)), interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i)) ],[MX(:,i) MX(:,i)],mc,'LineWidth',2,'LineStyle','-');
        end
        if plotmedian == 1
    ...
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)],mc,'LineWidth',2,'LineStyle','-');
        end
        if plotmedian == 1
