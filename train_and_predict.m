...
%    pause;
 ...
 case 'matlab_classify'
  tr=Dtest.supdat(supid,:)';
  if isfield(classifier_params,'minvar')
    [pred,cls.err,cls.posterior,cls.logp]=...
        classify_minvar(Dtest.dat',Dtrain.dat',Dtrain.supdat(supid,:)',classifier_params.dist_type, ...
                 classifier_params.prior,classifier_params.minvar);
  else
    [pred,cls.err,cls.posterior,cls.logp]=...
        classify(Dtest.dat',Dtrain.dat',Dtrain.supdat(supid,:)',classifier_params.dist_type, ...
                 classifier_params.prior);
  end
   cls.params=classifier_params;
   res=[];
   cls.logscore=log(cls.posterior(tr+1));
   nerr=nnz(tr~=pred);
 ...
 case 'sp_knn'
   ...
   Dtest=add_D_sup(Dtest,...
                   ...
                   pred');
 case 'codes'
  cutoff=classifier_params.cutoff;
  ranges=[-cutoff cutoff 0; -Inf -cutoff -1; cutoff Inf 1];
  
  X=Dtrain;
  X=discretize_D(X,struct('method','val','ranges',ranges));
  X=aggregate_D(X,'cols',classifier_params.supid,struct('method','mode_freq','freq',classifier_params.freq));
  X=filter_D_rows(X,'non_nan');
  X=filter_D_rows(X,struct('method','range','thresh',0.1));
  
  if size(Dtrain.dat,1)==0
    error('No genes left. Maybe reduce freq.');
  end

  % choose genes
  num_features=-1;
  fet_sel=struct('method','code_entropy','ranges',ranges,'freq',classifier_params.freq,'n_codes',...
                 classifier_params.n_codes,'use_base_entropy',classifier_params.use_base_entropy);
  [si,fetsel_res]=gg_sel_fet(X,supid,num_features,fet_sel);

  if isfield(classifier_params,'use_codes_as_train') && classifier_params.use_codes_as_train
    Dtrain=reorder_D_rows(X,si);
  else
    Dtrain=reorder_D_rows(Dtrain,X.gorigidx(si));
  end
  [Mt,m1,m2]=match_string_sets_hash(Dtest.gacc,Dtrain.gacc);
  Dtest=reorder_D_rows(Dtest,m1);
  % Dtest=reorder_D_rows(Dtest,si);
  
  [pred,tr,res,cls,Dtest,Dtrain]=train_and_predict(Dtrain,Dtest,supid,classifier_params.classifier);
  res.fet_sel.fetsel_res=fetsel_res;
  res.fet_sel.si=si;
 ...
 case 'gg_pnn'
   ...
   [c,cls.score,cls.sig,cls.logscore]=pnn_classify(Dtest.dat',Dtrain.dat',...
...
%    disp([ supid c cls.score cls.sig cls.logscore]);
 ...
 case 'gg_pnn_pca'
   ...
   [cls.pca.prj,cls.pca.m,cls.pca.D,cls.pca.V]=pca(Dtrain.dat',classifier_params.pca_k);
   test_dat=project_pca(Dtest.dat',cls.pca.m,cls.pca.V);
   tr=Dtest.supdat(supid,:)';
   
   if ~isfield(classifier_params,'class_weights')
     classifier_params.class_weights=[0.5; 0.5];
   end
   [c,cls.score,cls.sig,cls.logscore]=pnn_classify(test_dat,cls.pca.prj,...
                                                   [Dtrain.supdat(supid,:); 1-Dtrain.supdat(supid,:)],...
                                                   classifier_params.class_weights,classifier_params.sig_type,classifier_params.sig,classifier_params.dist_type,...
                                                   [Dtest.supdat(supid,:); 1-Dtest.supdat(supid,:)]);
%    disp([ supid c cls.score cls.sig cls.logscore]);
 ...
 case 'gg_knn'
  ...
  if ~isfield(classifier_params,'num_features')
    verbose('using all features');
    classifier_params.num_features=size(Dtrain.dat,1);
  end
  tr=Dtest.supdat(supid,:)';
  if length(supid)==1
 ...
 case 'classifier_w_sel_fet'
   ...
   tr=Dtest.supdat(supid,:)';
   [cls.si,cls.fetsel_res]=gg_sel_fet(Dtrain,supid, ...
                                      classifier_params.num_features,classifier_params.fet_sel);
   if classifier_params.num_features<=0
     classifier_params.num_features=length(cls.si);
   end
   ...
   save_cls=cls;
   classifier_params.method=classifier_params.classifier_type;
   [pred,tr,res,cls,Dtest,Dtrain]=train_and_predict(reorder_D_rows(Dtrain,1:classifier_params.num_features),...
                                                    reorder_D_rows(Dtest,1:classifier_params.num_features),supid,classifier_params);
   cls=add_struct(cls,save_cls);

 case 'classifier_w_preproc'
  if isfield(classifier_params,'params')
    classifier_params=add_struct(classifier_params,classifier_params.params);
  end
  
  if isfield(classifier_params,'preproc_train') && isfield(classifier_params,'preproc_test')
    Dtrain=preprocess_D(Dtrain,classifier_params.preproc_train);
    tmp=classifier_params.preproc_test;
    tmp.Dtrain=Dtrain;
    Dtest=preprocess_D(Dtest,classifier_params.preproc_test);
  else
    Dtrain=preprocess_D(Dtrain,classifier_params.preproc);
    Dtest=preprocess_D(Dtest,classifier_params.preproc);
  end
  
  classifier_params.method=classifier_params.classifier_type;
  [pred,tr,res,cls,Dtest,Dtrain]=train_and_predict(Dtrain,Dtest,supid,classifier_params);

 case 'sp_classifier'
  [typeacc,typedesc,Dtrain1,range,non_empty]=decollapse_supdat(Dtrain,supid,1:max(Dtrain.supdat(supid,:)));
  d=data(Dtrain1.dat',2*Dtrain1.supdat(range,:)'-1);
  eval(['[r,a]=train(' classifier_params.sp_classifier ',d);']);

  [typeacc,typedesc,Dtest1,range,non_empty]=decollapse_supdat(Dtest,1,1:max(Dtrain.supdat(1,:)));
  dtest=data(Dtest1.dat',2*Dtest1.supdat(range,:)'-1);
  rtest=test(a,dtest);
  tr=0.5*(get_y(rtest)+1);
  pred=0.5*(get_x(rtest)+1);
  cls=a;
  res=rtest;
  % update Dtrain, Dtest
 ...
 case 'gg_pnn_w_sel_fet'
   ...
   if isfield(classifier_params,'write_gct') && ...
         classifier_params.write_gct
     saveDtest=reorder_D_rows(Dtest,1:classifier_params.num_features);
     saveDtrain=reorder_D_rows(Dtrain,1:classifier_params.num_features);
     write_mit_gct_file(['Train_' num2str(supid) '.gct'], ...
                        saveDtrain);
     write_mit_cls_file(['Train_' num2str(supid) '.cls'], saveDtrain, ...
