TCGA=load_table('~/Projects/TumorInNormal/TCGA_Data/TCGA_tsv');
LUAD=cellfun(@isempty,strfind(TCGA.pair_id,'LUAD'));
TCGA=rmfield(TCGA,{'header','headline'});
LUAD_table=reorder_struct(TCGA,(LUAD==0));

M=load_struct(LUAD_table.maf_file_capture_deTiN{1});

for i=2:slength(LUAD_table)
    if ~isempty(LUAD_table.maf_file_capture_deTiN{i})
        m=load_struct(LUAD_table.maf_file_capture_deTiN{i});
        M=mergeStruct(m,M);
    end
end
M=rmfield(M,'N');
MutationSpectrumCategs(M);

c_LUAD_samples=LUAD_table.case_sample(LUAD_table.combined_deTiN_TiN_num>.01);


call=load_struct('/xchip/cga/gdac-prod/cga/jobResults/CallSomaticMutations/BDD-137-TP-NT-SM-PPSX-SM-PPSY/18485654/iteration1/BDD-137-TP-NT-SM-PPSX-SM-PPSY.call_stats.txt');
