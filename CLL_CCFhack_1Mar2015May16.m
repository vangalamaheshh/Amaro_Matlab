...
[i m]=ismember(A.sample,P.pair_id);
A.tid(i)=upper(P.case_sample(m(i)));
tabulate(A.tid)
...
A=reorder_struct(A,ismember(A.tid,X.tid));
T=reorder_struct(T,ismember(T.tid,X.tid));
G=reorder_struct(G,ismember(G.tid,X.tid));
...
for i=1:N
    ...
    for j=1:length(x1.pur)
        ...
        x1.ccf_median_hack(j,1)=round((x1.ccf_median_hack(j,1)*100))/100
        [q k]=max(x1.paft(j,:));
