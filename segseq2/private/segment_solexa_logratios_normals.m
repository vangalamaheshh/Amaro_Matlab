function [BKP,POS,R,pval,p_bkp,p_merge,stepTimes]=segment_solexa_logratios_normals( READN, READT, chrLengthFile, chrs, W, aN, aT, numInitFP, numFinalFP, flagSpeedup )
...
step_time=toc;
fprintf(1,'\n1) Normal read statistics:  %f',step_time);
stepTimes(1) = step_time;

tic
[BKP,p_bkp]=filter_pval_numFP( W, pval, CHR, POS, IDXN, numInitFP );
step_time=toc;
fprintf(1,'\n2) Filter 1000 FP initial normal breakpoints:  %f',step_time);
stepTimes(2) = step_time;

tic
  SEG=initialize_seg_logratios( chrLength, CHR, BKP, aN, aT, POS, CUMULN, CUMULT );
t=toc;
fprintf(1,'\n3) Initialize normal segments:  %f',step_time);
stepTimes(3) = step_time;
