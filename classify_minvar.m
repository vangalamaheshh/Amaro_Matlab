function [outclass, err, posterior, logp] = classify_minvar(sample, training, group, type, prior, minvar)
...
%   $Revision: 2.15.4.4 $  $Date: 2004/12/06 16:37:10 $
...
if exist('minvar','var') && ~(strcmp(lower(type),'diagquadratic') || ...
                              strcmp(lower(type),'diaglinear') || ...
                              strcmp(lower(type),'diaglinear_median'));
  error('Minvar works only with DiagQuadratic or DiagLinear');
end
...
if nargin < 4 || isempty(type)
    type = 'linear';
elseif ischar(type)
    types = {'linear','quadratic','diaglinear','diaglinear_median','diagquadratic','mahalanobis'};
...
case 'diaglinear'
    ...
    if exist('minvar','var')
      tmp=find(S<sqrt(minvar));
      if ~isempty(tmp)
%        fprintf(1,['(' num2str(length(tmp)) '/' num2str(length(S)) ')']);
        S(tmp)=sqrt(minvar);
      end
    end
...
%        fprintf(1,['(' num2str(length(tmp)) '/' num2str(length(S)) ')']);
    ...
    for k = 1:ngroups
        A = (sample - repmat(gmeans(k,:), mm, 1))./repmat(S,mm,1);
        D(:,k) = log(prior(k)) - .5*(size(A,2)*median(A .* A, 2) + logDetSigma);
    end
...
case {'quadratic' 'mahalanobis'}
    ...
    for k = 1:ngroups
        ...
        % of R.
        [Q,R] = qr(training(gindex==k,:) - repmat(gmeans(k,:), gsize(k), 1), 0);
