function sp = plot_snp_score(base_dir,ext,C,q,ads,q_thresh,use_loglog,write_pdf,plot_type,...
...
%
%   SP = PLOT_SNP_SCORE(BASE_DIR,FNAME_SUFFIX,C,Q,ADS,Q_THRESH,...
...
%   C - structure containing copy number data
...
if exist('cyto','var')
    ...
    C=rmfield_if_exists(C,{'cyton','armn','cyto_stain'});
    C=add_cyto(C,c);
end
...
%!    ymax = qscale(qv_scale(k));
  ...
  for ci=2:2:max(C.chrn)
    inchr=find(C.chrn==ci);
    if ~isempty(inchr)
      mnc=min(inchr);
      mxc=max(inchr);
...
%!eliminate dependence on printD->print_pdf->ps2pdf which has hard-wired 
%!server paths - we think no one uses the v2 pdf anyway
%!  print_D([name '.v2'],{{'pdf'}});
