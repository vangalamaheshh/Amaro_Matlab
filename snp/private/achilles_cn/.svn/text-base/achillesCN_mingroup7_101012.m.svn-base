...
%% (1) unfiltered permutations of amplifications (with Z-scores)
...
R.amp = perform_permutations(R,@(x,y) mean(x,1)-mean(y,1),ZAC.dat(:,acx),anyamps,noamps,classes,permtest_options);
% save intermediate results
n = write_hp_permstats([output_dir DATA_ID '.allamps.HP.ZSming7.LR.bin.101012.tab'],R.amp,gmatch,ZAC,G,cyto,'any amp','no amp');
...
permtest_options.tail = 'leftright';
permtest_options.min_group = 7;
R.del = perform_permutations(R,@(x,y) mean(x,1)-mean(y,1),ZAC.dat(:,acx),anydels,nodels,classes,permtest_options);
% save intermediate results
n = write_hp_permstats([output_dir DATA_ID '.alldels.HP.ZSming7.LR.bin.101012.tab'],R.del,gmatch,ZAC,G,cyto,'any del','no del');
...
permtest_options.min_group = 7;
tic
R.neg = perform_perms_parallel(R,@(x,y) mean(x,1)-mean(y,1),SAC.dat(:,acx),anyamps,noamps,classes,permtest_options);
toc
% save intermediate results
n = write_hp_permstats([output_dir DATA_ID '.allamps.HP.scrambled.LR.bin.101012.tab'],R.neg,gmatch,SAC,G,cyto,'any amp','no amp');
verbose('wrote %d unfiltered deletion permutation statistics across hairpins',20,n);
...
toc
% save intermediate results
n = write_hp_permstats([output_dir DATA_ID '.alldels.HP.scrambled.LR.bin.101012.tab'],R.negdel,gmatch,SAC,G,cyto,'any del','no del');
