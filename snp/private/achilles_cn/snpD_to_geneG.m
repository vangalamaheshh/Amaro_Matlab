function G = snpD_to_geneG(D,rg,rg_field,options)
...
%!    end
    ...
    for ag = 1:length(uqids)
        ...
        rgindex{ag} = matches;
        if ~isempty(matches)
            % loop over all matches to unigue gene ID
            for k = matches'
...
%!                     'snp_end',num2cell(snp_end) );
    ...
    for field = matrix_fields
        fld = char(field);
        if isfield(D,fld)
            G.(fld) = nan(length(firstsnps),size(D.(fld),2));
            % for speed, undo any SegArrays
            if isa(D.(fld),'SegArray')
                dat = full(D.(fld));
            end
            for i = 1:length(snps)
                % use one of several methods of compressing the data
                switch options.collapse_method
                  case 'mean'
                    G.(fld)(i,:) = mean(dat(snps{i},:),1);
                  case 'median'
                    G.(fld)(i,:) = median(dat(snps{i},:),1);
                  case 'min'
                      if length(snps{i})>1
                        G.(fld)(i,:) = min(dat(snps{i},:));
                      else
                        G.(fld)(i,:) = dat(snps{i},:);
                      end
                  case 'max'
                    G.(fld)(i,:) = max(dat(snps{i},:),1);
                  case 'extreme'
                    min_val = min(dat(snps{i},:),1);
                    max_val = min(dat(snps{i},:),1);
                  ...
                  case 'all'
                    G.(fld)(i,:) = NaN;
                    same_mask = all(diff(dat(snps{i},:),1,1)==0,1);
                    G.(fld)(i,same_mask) = dat(snps{i}(1),same_mask);
                end
            end
        end
    end
