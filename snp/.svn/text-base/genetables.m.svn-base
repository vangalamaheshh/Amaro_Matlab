function tab = genetables(CL21r,A,~,rg,cyto,regs,calls,~,ext,...
                          ~,base_dir,genepattern,partial_hits,fname)
%GENETABLES Write Amp_genes, Del_genes, Table_Amp and Table_Del files
...
%%%%%from snp_generate_region_data and genelist code  %%%
%
% ---
% $Id$
...
for k=1:length(calls)  %calls is cell with 1/0 amplifications{1} and deletions{2} data
  ...
  for pki=1:size(calls{k},1)  % :-1:1  %loop over peaks
    ...
    [~,~,enl_region_to_next_snp]=genomic_location(CL21r,{[regs{k}(pki).enl_peak_st ...
                        regs{k}(pki).enl_peak_en]},cyto,1);
    [~,~,peak_region_to_next_snp]=genomic_location(CL21r,{[regs{k}(pki).peak_st ...
                        regs{k}(pki).peak_en]},cyto,1);
    %Get genes in region
    [genes_in_region,closest_to_region]=genes_at(rg,CL21r.chrn(regs{k}(pki).enl_peak_st), ...
                             enl_region_to_next_snp{1}(1), enl_region_to_next_snp{1}(2),1,partial_hits(k));
    [genes_in_peak,closest_to_peak]=genes_at(rg,CL21r.chrn(regs{k}(pki).peak_st), ...
                           peak_region_to_next_snp{1}(1),peak_region_to_next_snp{1}(2),1,partial_hits(k));
    %%% unique the above by locuslink id
    ...
    if ~isempty(genes_in_peak)
      lid_in_peak = cat(1,rg(genes_in_peak).locus_id);
      [~,un_lid_pk_i1,~] = unique(lid_in_peak);
      genes_in_peak=genes_in_peak(un_lid_pk_i1);
    end
...
%% output the table_{amp|del}.txt (formerly Table_{Amp|Del}.txt) file
...
for k = 1:length(regs)  %Do for amps and dels
  ...
  if ~isempty(regs{k})
    ...
    % get cytoband names
    lesionlocs = double(CL21r.pos([regs{k}.peak_st])') + 1e11.*double([regs{k}.chrn]);  %where the lesions start
    ...
    tmp_cell=genomic_location(CL21r,mat2cell([cat(1,regs{k}.peak_wide_st) ...
