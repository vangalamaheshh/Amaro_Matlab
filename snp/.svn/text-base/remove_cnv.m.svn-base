function [C,nsnpsremoved,m2] = remove_cnv(C,cnv_file,byposition)
...
if iscell(cnv_file)
    ...
    for i=1:numel(cnv_file)
        [C,~,idx] = remove_cnv(C,cnv_file{i},byposition);
        m2 = union(m2,idx);
    end
...
if byposition
    ...
    hasheader = ~any(strcmp(firstline{1},C.marker));
end
...
if byposition
    ...
    markerpos = double(C.chrn)*1e11 + double(C.pos);
    ...
    fclose(fid);
    % match markers
    [~,m1,m2]=intersect(cnv{1},C.marker);
    ...
    C = reorder_D_rows(C,setdiff(1:size(C.dat,1),m2));
end
