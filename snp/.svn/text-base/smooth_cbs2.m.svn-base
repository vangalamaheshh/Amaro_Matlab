function C=smooth_cbs2(C,n,set_to_val)
%SMOOTH_CBS2 - join small CN segments to larger ones (LOH GISTIC variant)
%
%  D = SMOOTH_CBS(D,N,SET_TO_VAL)
%
%  The segmented data in D.cbs is processed by joining each segment of
%  length N snps or fewer to its largest adjacent neighbor. If SET_TO_VAL
%  is not given or is empty, then the joined segment is a weighted average
%  of the two comprising segments, otherwise, the value of small segments
%  are set to SET_TO_VAL.
...
if ~isfield(C,'cbs_rl') || (isfield(C,'cbs_rl') && ...
        isempty(C.cbs_rl))
    ...
    for c=smooth_chromosomes
        ...
        if any(small)
            if exist('set_to_val','var') && ~isempty(set_to_val)
                ...
                not_nan=find(~isnan(rl_chr(:,3)));
                rl_chr=rl_chr(not_nan,:);
                sz=rl_chr(:,2)-rl_chr(:,1)+1;
                ...
                while ~isempty(smalls)
                    ...
                    if mn(1)<mn(2) || isnan(mn(1))  %quick fix to deal with NaNs -- make better
                        C=combine_segs(C,i,in_chr(not_nan(smalls(mi(1)))),in_chr(not_nan(smalls(mi(1))-1)));
                    else
                        C=combine_segs(C,i,in_chr(not_nan(smalls(mi(2)))),in_chr(not_nan(smalls(mi(2))+1)));
                    end
