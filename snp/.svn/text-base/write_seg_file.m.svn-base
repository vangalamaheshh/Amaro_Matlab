...
for i=1:length(rl)
  if report_nan_segments
    ...
    idx=find(~isnan(rl{i}(:,3)));
  end
  % segment-by-segment within sample
  for j=idx'
    if convert_to_raw
      fprintf(f,'%s\t%s\t%d\t%d\t%d\t%f\n',char(D.sdesc(i,:)),num2chromosome(D.chrn(rl{i}(j,1))),...
              D.pos(rl{i}(j,1)),D.pos(rl{i}(j,2)),rl{i}(j,2)-rl{i}(j,1)+1,2^(rl{i}(j,3)+1));
    else
      fprintf(f,'%s\t%s\t%d\t%d\t%d\t%f\n',char(D.sdesc(i,:)),num2chromosome(D.chrn(rl{i}(j,1))),...
              D.pos(rl{i}(j,1)),D.pos(rl{i}(j,2)),rl{i}(j,2)-rl{i}(j,1)+1,rl{i}(j,3));
    end
  end
end
fclose(f);
