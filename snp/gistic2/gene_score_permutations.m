function [rg d sizes] = gene_score_permutations(C,y,rg,res)
...
%---
% $Id$
% $Date: 2013-07-26 15:42:06 -0400 (Fri, 26 Jul 2013) $
% $LastChangedBy: schum $
% $Rev$
...
for j = 1:sizes_len
  ...
  if sizes(j) ~= 0 %do only windows of the same size as at least one gene
    if j > 1 %no need to test window size == 1 (that's the regular histogram)
