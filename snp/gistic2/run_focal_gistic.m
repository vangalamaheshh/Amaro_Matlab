function [D regs params] = run_focal_gistic(base_dir,D_file,cyto_file,params)
% RUN_FOCAL_GISTIC -- Perform Focal GISTIC2.0 pipeline
%
% D = run_focal_gistic(base_dir,D_file,cyto_file,params)   
%
% Required inputs:
%     base_dir: directory in which to write output for this run
%     D_file: either file pointing to a saved D_struct, or D_struct itself
%     cyto_file: refgene/cytoband file location 
...
% $Id$
% $Date: 2013-01-18 14:31:50 -0500 (Fri, 18 Jan 2013) $
% $LastChangedBy: dicara $
% $Revision:$ 
  ...
  if ~exist('base_dir','var') || isempty(base_dir)
    error('Must supply valid base directory...');
  else
    base_dir = add_slash_if_needed(base_dir);  
    if ~exist(base_dir,'dir')
      mkdir(base_dir)
    end
%!  cd(base_dir); %! necessary?
  end
  ...
  if ~exist('D_file','var') || isempty(D_file)
    error('Must supply valid D struct or D struct file...');
  else
    if isstruct(D_file)
      ...
      verbose('Loading D...',20);
      D = load_D(D_file);
    end
  end
  params.use_segarray = isa(D.dat,'SegArray');

  if ~exist('cyto_file','var') || isempty(cyto_file)
    error('Must supply valid cytoband/refgene file...');
  else
    verbose(['Loading cytoband/refgene file: ''',cyto_file,''''],20);
    load(cyto_file);
  end
  
  if params.res == .05
    params.res = .05/size(D.dat,2);
  end
  ...
  if ~isempty(params.array_list_file)
    ...
    n = length(use_arrays);
    score_type.res = score_type.res*orig_n/n;
  ...
  [q,p,d,ads,score_thresh,gene_gistic_rg,gene_gistic_ds] = compute_stats(D,scores,rg,score_type,...
  ...
  if ~params.do_gene_gistic
    [regs seg_assignment] = identify_peaks_by_arbitration(Z,focQs,ads,q, ...
                            score_thresh,[],[],[],params.do_arbitration,params.arm_peeloff);
  else
    [regs seg_assignment] = identify_peaks_by_arbitration(Z,focQs,ads,q,score_thresh, ...
                            gene_gistic_rg,gene_gistic_ds,score_type.res*params.alpha(2),...
  ...
  if params.do_gene_gistic
    clean_struct = struct('clean_up',1,'seg_assignment',seg_assignment, ...
                          'score_type',score_type,'d',{d{1} gene_gistic_ds{1}},'alpha',params.alpha);
  else
    clean_struct = struct('clean_up',1,'seg_assignment',seg_assignment, ...
  ...
  for l=1:ncl
    ...
    if ~params.do_gene_gistic
      ...
      wregs{l} = calculate_wide_peaks_for_regs(Z,focQs,regs,perm_ads,params.conf_level(l),...
                                               score_thresh,score_type,range_struct,...
                                               gene_gistic_rg,params.peak_types);
    end
  end
  ...
  if params.do_gene_gistic
    q{2} = interpolate_gene_scores(D,gene_gistic_rg,'q',@min);
    ads{2} = interpolate_gene_scores(D,gene_gistic_rg,'gene_scores',@max);
  end
