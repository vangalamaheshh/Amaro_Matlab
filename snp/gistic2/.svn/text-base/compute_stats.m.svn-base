function [q,p,focal_d,ads,score_thresh,gene_gistic_rg,gene_gistic_ds] = compute_stats(D,scores,rg,score_type,alpha,qv_thresh,do_gene_gistic)
...
% See the accompanying file LICENSE.txt for licensing details.
  ...
  if do_gene_gistic
    ...
    [gene_gistic_rg gene_gistic_ds] = gene_score_permutations(D, ...
                          scores.del-min_valD,rg,alpha(2)* score_type.res);
    gene_gistic_rg = order_rg_by_pos(gene_gistic_rg);

    gene_gistic_rg = gene_gistic_rg([gene_gistic_rg.p] >= 0);
    gene_p = [gene_gistic_rg.p];
    gene_q = calc_fdr_value(gene_p);
    gene_scores = score_from_p(gene_gistic_ds{1},gene_p)*score_type.res*alpha(2);
    for i=1:length(gene_gistic_rg)
      gene_gistic_rg(i).gene_scores = gene_scores(i);
      gene_gistic_rg(i).q = gene_q(i);
    end
