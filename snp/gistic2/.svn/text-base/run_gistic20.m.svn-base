function [D focal_regs params] = run_gistic20(base_dir,CL,ref_gene_file,params)
%RUN_GISTIC20 run focal and broad GISTIC analyses and save results
%
%  [D focal_regs params] = run_gistic20(BASE_DIR,D,REF_GENE_FILE,PARAMS)
%
...
%
%  REF_GENE_FILE is the path to to a matlab reference gene file.
%
...
% See the accompanying file LICENSE.txt for licensing details.
  ...
  if ~exist('CL','var') || isempty(CL)
    ...
    if ~isstruct(CL)
      if ~exist(CL,'file') 
        error('Must supply valid D file or struct!')
      end
    end
  end
  ...
  [D focal_regs params] = run_focal_gistic(base_dir,CL,ref_gene_file,params);
  ...
  if params.run_broad_analysis
    ...
    [arm_medians qA qD zA zD fA fD names num_genes] = gistic_broad_analysis(base_dir,D,ref_gene_file,params);
  end
  ...
  if params.write_gene_files
    ...
    if exist('arm_medians','var') && ~isempty(arm_medians)
      write_arm_medians([pathbase 'broad_values_by_arm' extxt],D,arm_medians,names);
    end    
    
    % load and clean up the reference genome file
    load(ref_gene_file);
    % remove haplotype and other weird annotations
    bad_ref_genes = [rg.chrn] > 1000;
    ...
    %% Write gene calls
    gene_calls(base_dir,params,ref_gene_file);
