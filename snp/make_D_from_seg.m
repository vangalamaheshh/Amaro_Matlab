...
fid = fopen(segfile);
if fid < 0
    throw(MException('snp:make_D_from_seg:cnvfileOpenError',...
...
if any(isnan(segschrn))
    ...
    throw(MException('snp:make_D_from_seg:BadSegment',...
        ...
        sum(isnan(segschrn)),segfile,segs{2}{badline},badline+hasheader));
end
...
if any(mismatch_beg) || any(mismatch_end)
    ...
    throw(MException('snp:make_D_from_seg:BadSegment',...
        ...
        'First bad position is chr%s:%d at line %d.'],...
        Nmismatch,segfile,markersfile,segs{2}{badline},badpos,badline+hasheader));
end
...
if any(osizes > 0)
    if max(osizes) > 1
        ...
        if any(begends(:,2) - begends(:,1) < 0)
            ...
            badlines = sort(order(find(begends(:,2)-begends(:,1) < 0))) + hasheader;
            throw(MException('snp:make_D_from_seg:zeroLengthSegment',...
                    ['%d segments shortened to length zero in file ''%s''.\n' ...
                     'First segment shortened to zero is at line %d.'],...
                    length(badlines),segfile,badlines(1)));
        end
    end
end
...
if options.use_segarray
    ...
    D.dat = SegArray.fromSegments(segmap(st1),segmap(en1),sample1,segs{5},...
                                  NaN,length(D.pos),length(D.sdesc));
else
    % create a full array for D.dat
    D.dat=nan(length(D.pos),length(D.sdesc));
    for i=1:length(D.sdesc)
        segidx=find(sample1==i); % all segments for sample i
        for j=1:length(segidx)
            s=segidx(j);
            D.dat(segmap(st1(s)):segmap(en1(s)),i)=segs{5}(s);
        end
        if mod(i,50)==0
            disp(i);
        end
