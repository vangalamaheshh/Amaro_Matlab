function gistic_pipeline(select_type,snpdir,data_fname,basedir,...
                         datadir,sample_info_dir,rundir,GI_fname,sample_info_fname,skip_snp,refgene_file)
...
for i=1:length(data_fname)
  ...
  if exist(mat_file,'file')
    ...
    M{i}{1}=read_modelled_data_file([ snpdir data_fname{i}],-1,1,1);
    x=M{i}{1};
    save(mat_file,'x');
  end
  [M{i}{1},IDX{i}{1}]=gistic_filter_samples(M{i}{1},SI,select_type);
end
...
use_samples_in=setdiff(setdiff(find(C2.supdat(find_supid(C2,'REP'),:)==0),throw_out),...
                       strmatch('cell line',{C2.sis.cellxeno}));
disp('throwing out cell lines');
...
par=struct('qlambda',0.999,'lambdabreak',8,'lambdacluster',8,...
           'lambdaclusterGen',40,'param_d',6,'bandwidth',10,'add_cols',[13 14]);
run_glad_batch(C2,0,1,'normal',1,par);
...
for i=1:size(C2.dat,2)
  ...
  for j=2:length(seg)
    fprintf(f,['%d %s %s %s %s %s %s %s\n'],i,seg{j}{2},seg{j}{3},seg{j}{4},seg{j}{5},seg{j}{6},seg{j}{7},seg{j}{8});
  end
end
fclose(f);
...
%cd(cbs_dir);
ts=[0.1 0.1];
[CL21,regs,pvs]=after_segmentation(C2,1,0,0,idx,4,[],[],[],[],['./output'],1,[],ts,1);
...
% mean(mean(CL.dat(find(CL.chrn~=23),ctrl_i)>ts(1))) % 0.0093 % 0.06 --> 0.0088
% mean(mean(CL.dat(find(CL.chrn~=23),ctrl_i)<-ts(2))) % 0.0099% 0.08 --> 0.0093
...
if exist('refgene_file','var')
  ...
  make_all_lesions_file(['all_lesions.' method_st '.txt'],CL21,regs,pvs,cyto,[],ts);
...
%  missing1=setdiff(1:length(fsymb),unique(m1));
  ...
  calls=call_regs(CL21,regs,{ts});
  tab=snp_generate_region_data(CL21,[],[],rg,regs,calls,ts,pvs);
