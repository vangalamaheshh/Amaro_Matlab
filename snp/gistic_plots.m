function gistic_plots(base_dir,ext,CL21,q,ads,regs,cyto,all_lesions_file,qv_scale,...
        aLabels,dLabels,aTopLabels,dTopLabels,qt,ull,wp,no_stripes,add_broad_bars,...
...
if ~isempty(regs) && size(regs{1},2)>0
    ...
    reg_names{1} = {cyto(CL.cyton(amp_Peaks_Sorted)).name};
else
    reg_names{1}=[];
end
...
if ~isempty(regs) && size(regs{2},2)>0
    ...
    reg_names{2} = {cyto(CL.cyton(del_Peaks_Sorted)).name};
else
    reg_names{2}=[];
end
...
plot_snp_score( base_dir, ext, CL, q, ads,qt,ull,wp,'vert',cyto,qv_scale,qv_ticks, ...
