SEG=load_table('~/Documents/AggregateStilgenbauerAllelic.seg');
SEG.Sample=SEG.sample;
...
for i=1:length(Is)  
    ...
    if I_seg.tau(j)<1.85 
        ...
        c=255*(.5-(2-I_seg.tau(j)))/255;
        patch(x1,y1,[c c 255/255],'edgecolor','none')   
        end
    elseif I_seg.tau(j)>2.15
        y1=[ I_seg.xStart(j) I_seg.xStart(j) I_seg.xEnd(j) I_seg.xEnd(j) I_seg.xStart(j)];
        x1=[s-dx s+dx s+dx s-dx s-dx];
        if I_seg.tau(j)>2.5
                        patch(x1,y1,[1 0 0],'edgecolor','none')
        else
            c=255*(.5-(I_seg.tau(j)-2))/255;
