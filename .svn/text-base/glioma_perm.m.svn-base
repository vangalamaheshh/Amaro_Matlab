function [h_amp,h_del,naamp,nadel]=glioma_perm(Csmooth,niter,method)
...
for j=1:niter
  ...
  for k=1:length(t1s)
    ...
    y=naamp;
    y(isnan(y))=0;
    h_amp(:,k)=h_amp(:,k)+histc(full(y),0:0.1:100);
    
    %  y=round(nadel*10)/10;
    y=nadel;
    y(isnan(y))=0;
    h_del(:,k)=h_del(:,k)+histc(full(y),0:0.1:100);
  end
