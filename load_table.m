...
while ~OK
    try
        if set_maxlines
            tab.dat=textscan(fid,format,maxlines,'HeaderLines',headlines,'Delimiter',dlm); %,varargin{:});
        else
            tab.dat=textscan(fid,format,'HeaderLines',headlines,'Delimiter',dlm); %,varargin{:});
        end
        OK = true;
