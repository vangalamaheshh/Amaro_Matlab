...
classdef DirichMix < handle    
    properties
        ...
        beta  = .05*eye(4);   % Gauss-Gamma param  % only affect the variance of mu
        mu0   = [0,0,0,0];       % Gauss-Gamma param  % only affect the mean of mu
        ...
        function DoIteration(obj,iter)
            ...
            while(iter)
                ...
                for n1 = 1:obj.N
                    ...
                    for k = 1:obj.K
                        catp(k) = obj.cn(k)/(obj.N-1+obj.alpha)*DirichMix.norm2dpdf(obj.data(n,:),obj.mu(k,:),obj.sig);
                    end
                    
                    % computer partition function
                    mu_    = obj.mu;           % theoretically, could be any value
                    ...
                    a0_    = obj.a+1;
                    mu0_   = ((obj.beta+eye(size(obj.data,2)))\(obj.beta*obj.mu0'+eye(size(obj.data,2))*y_'))'; % error on size of eye
                    beta0_ = (obj.beta+eye(size(obj.data,2)))/sig_^2;
                ...
                for k = 1:obj.K
                    ...
                    % posterior hyperparameter
                    a_    = obj.a+yN*2/2;
                    mu_   = ((obj.beta+yN*eye(size(obj.data,2)))\(obj.beta*obj.mu0'+yN*eye(size(obj.data,2))*mean(y0,1)'))';
                    b_    = obj.b+.5*trace(...
                        (y0-repmat(mean(y0,1),yN,1))*eye(size(obj.data,2))*(y0-repmat(mean(y0,1),yN,1))'...
					...
					beta_ = obj.sig(k)^2\(obj.beta+yN*eye(size(obj.data,2)));
    ...
    methods(Static)
        function mu  = MultivarStudentRnd(a,b,beta,mu)
			...
			y   = randn(1,size(beta,2))*sigma;
			% u ~ chi-square(n)        
			u   = chi2rnd(size(beta,2))*a;
