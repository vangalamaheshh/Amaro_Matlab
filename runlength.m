...
if isempty(x)
  ...
  if min(size(x))>1
    ...
    for j=1:size(segments,1)  %loop over rows of segments vector  (1)
      ...
      [u,ui,uj]=unique(y);  %u(uj) = y for sorted u, so uj gives rank of each element in y
      ...
      ch=find(abs(diff([0 uj 0])));  %ch(i) is 1 if uj(i+1)-uj(i)~=0 (if input x is 1,0,-1 for pos, zero and neg slope, ch gives indices of change in x)
