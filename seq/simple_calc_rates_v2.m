function M = simple_calc_rates_v2(M,P)
...
if haveintron
  M.Nintron_c = round(nansum(nansum(M.N_intron_cov,3),1));
else
  M.Nintron_c = nan(1,ncat);
end
if haveflank
  M.Nflank_c = round(nansum(nansum(M.N_flank_cov,3),1));
else
  M.Nflank_c = nan(1,ncat);
end
M.Nsil_c = round(nansum(nansum(M.N_sil_cov,3),1));
M.Nsf_c = M.Nflank_c + M.Nsil_c;
M.Nnon_c = round(nansum(nansum(M.N_non_cov,3),1));

if haveintron
  M.nintron_c = round(nansum(nansum(M.n_intron,3),1));
else
  M.nintron_c = nan(1,ncat);
end
if haveflank
  M.nflank_c = round(nansum(nansum(M.n_flank,3),1));
else
  M.nflank_c = nan(1,ncat);
end
M.nsil_c = round(nansum(nansum(M.n_silent,3),1));
M.nsf_c = M.nflank_c + M.nsil_c;
M.nnon2_c = round(nansum(nansum(M.n_nonsilent_ignoring_null_categ,3),1));
M.nnon_c = round(nansum(nansum(M.n_nonsilent,3),1));
...
if haveintron
  M.gene.Nintron_c = squeeze(round(nansum(M.N_intron_cov,3)));
else
  M.gene.Nintron_c = nan(ng,ncat);
end
if haveflank
  M.gene.Nflank_c = squeeze(round(nansum(M.N_flank_cov,3)));
else
  M.gene.Nflank_c = nan(ng,ncat);
end
M.gene.Nsil_c = squeeze(round(nansum(M.N_sil_cov,3)));
M.gene.Nsf_c = M.gene.Nflank_c + M.gene.Nsil_c;
M.gene.Nsphere_c = nan(ng,ncat);
M.gene.Nnon_c = squeeze(round(nansum(M.N_non_cov,3)));
...
M.gene.nbagelsil_c = zeros(ng,ncat);
if haveintron
  M.gene.nintron_c = squeeze(round(nansum(M.n_intron,3)));
else
  M.gene.nintron_c = nan(ng,ncat);
end
if haveflank
  M.gene.nflank_c = squeeze(round(nansum(M.n_flank,3)));
else
  M.gene.nflank_c = nan(ng,ncat);
end
M.gene.nsil_c = squeeze(round(nansum(M.n_silent,3)));
M.gene.nsf_c = M.gene.nflank_c + M.gene.nsil_c;
M.gene.nsphere_c = nan(ng,ncat);
M.gene.nnon_c = squeeze(round(nansum(M.n_nonsilent,3)));
...
if method==4
  ...
  if P.use_bagels
    ...
    bkgd = [fintron_c fflank_c fsil_c fsf_c];
  end
...
if isfield(M.gene,'desc'), M.g.desc = M.gene.desc; end
if isfield(M.gene,'longname'), M.g.desc = M.gene.longname; end

M.g.N = M.gene.Nnon_c(:,end);

if any(~isnan(M.gene.nintron_c))
...
for i=1:M.ncat
  M.g = setfield(M.g,['n' num2str(i)],M.gene.nnon_c(:,i));
end
