...
headers = [{'case'}; {'control'}; {'N'}; {'N_A'}; {'N_nA'};{'N_filtered'};{'N_oxo'}; {'N_oxo_CI_low'}; {'N_oxo_CI_high'};{'N_mut'}; {'RR'}; {'RR_CI_low'}; {'RR_CI_hi'}; {'PA'}; {'PA_CI_low'}; {'PA_CI_hi'};{'PoxoG'};{'PoxoG_Est_CI_low'};{'PoxoG_Est_CI_hi'};{'Proportion_N_filt'};];
...
for i = 1:length(pairs)
    ...
    tbl_entry = [strjoin('\t', [{pair.case}; {pair.control};{num2str(length(M.foxog))}; {num2str(length(M_A.foxog))}; ...
        ...
        {num2str(PoxoGEst)}; {num2str(PoxoGEstci(1))}; {num2str(PoxoGEstci(2))} ;{num2str( sum(fdrStructs(i).cut)/length(M.foxog) )} ...
        ]) '\n'];
    fprintf(1,  tbl_entry)
...
if isGeneratingPlots
    ...
    plotMutationSpectrumCategLego1(mafTable,'unit',[outputFigureDir outputMAFFilename '_lego_before'], Pbefore);
    Pafter.label = [basename ' Filtered'];
    plotMutationSpectrumCategLego1(trimStruct(mafTable, ~allFdrStructs.cut),'unit',[outputFigureDir outputMAFFilename '_lego_after'], Pafter)
    disp('Summary Lego plots completed.')
    
    summaryTable = load_table([outputTableDir 'caseTableData.tsv'], char(9));
    figure;
