...
% Add the headers to the mafTable struct so that printStruct will append
%   automatically.
...
mafTable.header(length(mafTable.header)+1) = {'pox'};
mafTable.header(length(mafTable.header)+1) = {'qox'};
mafTable.header(length(mafTable.header)+1) = {'isArtifactMode'};
mafTable.header(length(mafTable.header)+1) = {'oxoGCut'};

% Write a file that contains all of the mutations 
printStruct(mafTable, 1:length(mafTable.Start_position), [outputMAFFilename '.all.maf.annotated']);

% Write a file that only contains passing mutations
passingMafTable = trimStruct(mafTable, ~mafTable.oxoGCut);
printStruct(passingMafTable, 1:length(passingMafTable.Start_position), outputMAFFilename);
