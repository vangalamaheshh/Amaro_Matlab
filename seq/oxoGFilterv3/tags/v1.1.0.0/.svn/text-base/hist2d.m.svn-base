...
if (nargout == 0)|| (nargout>3)
    ...
    if (P.MARGINHISTS)
        subplot('position',[0.25 0.25 0.65 0.65])
    end
    ...
    if (P.MARGINHISTS)
        ...
        subplot('position',[0.1 0.25 0.12 0.65])
        h1(3)=barh(yc(:,1),sum(zc,2),'hist');
        hh(3)=get(h1(3),'parent');
        ylim(ylim1)
        p1=get(hh(1),'position');
        subplot('position',[0.25 0.085 p1(3) 0.12])
        h1(4)=bar(xc(1,:),sum(zc,1),'hist');
