...
for ac=1:NA
    ...
    NLL = @(a) sum(-log10(poisspdf(n',N1*(1-a)*n1+N1*a*n2)));
    [alf(ac),fval0] = fminbnd(NLL,0,1);
    NLL0 = @(a) sum(-log10(poisspdf(n',N1*(1-a)*n1+N1*a*n2)))-(fval0+dLL);
    ...
    % reduce search region to small interval where NLL0 crosses zero 
    % but doesn't blow up (CS 21 Feb 2013) 
    av=[0 10.^[-8:-3] .1:0.1:1];
    for i=1:length(av)
        fv(i,1)=NLL0(av(i));
    end
...
if lod0 < 0
    warning(['Final lod0 is a negative number (' num2str(lod0) ').   Setting it to zero.'])
    lod0 = 0;
end
