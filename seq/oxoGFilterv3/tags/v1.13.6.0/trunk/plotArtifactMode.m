...
oxoQstring='';
if isfield(M_A,'i_picard_oxoQ')
    uQ=unique(M_A.i_picard_oxoQ);
    if length(uQ)==1
        oxoQstring=sprintf(',  picard oxoQ=%.2f',uQ);
    end
end
text(.05, .85, [{'OxoG Mode'};{['n = ' num2str(length(M_A.foxog)) oxoQstring]};{filteredMutationsString};{filteredMutationsStringLine2};{filteredMutationsStringLine3}], 'FontSize', 14, 'Units', 'Normalized', 'Parent', h(1))
set(h(1), 'FontSize', 12)
