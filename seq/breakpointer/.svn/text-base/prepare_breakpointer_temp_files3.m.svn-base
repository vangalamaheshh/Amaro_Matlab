function endat=prepare_breakpointer_temp_files3(drangerfile,bamfile,startfrom,endat,P)
...
%        fprintf(fid,'%s\n',upper(seq));
%    end
fprintf(fid,'%s\n',upper(genome_region(X.chr1(i),X.pos1(i)-P.expand_seq_around_bkpt-P.readlen+1,X.pos1(i)+P.expand_seq_around_bkpt+P.readlen,P.refdir)));
fprintf(fid,'%s\n',upper(genome_region(X.chr2(i),X.pos2(i)-P.expand_seq_around_bkpt-P.readlen+1,X.pos2(i)+P.expand_seq_around_bkpt+P.readlen,P.refdir)));
    fclose(fid);
    if (X.str1(i)==0)
      shift_by_ins1=-1;
    else
