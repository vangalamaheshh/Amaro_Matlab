function fh_BreakPointerScatter(sample,drr,bam,blacklist,refdir,insertionsize_input,fish_no_supp_reads_thres,fish_low_confidence_fixsidewithread,fish_low_confidence_fixsidewithoutread,fish_high_confidence_fixsidewithread,fish_high_confidence_fixsidewithoutread,fish_allowedmm,fish_tipsize,fish_minmmintip,fish_maxN,fish_expand_pairs_extraction,fish_maxreads,readlen,align_enough_reads,align_wsplit,align_min_qual,libdir,firstidx,lastidx)
...
if ~isnumeric(insertionsize_input)
    if isempty(strfind(insertionsize_input,'.isz'))
        if isempty(strfind(insertionsize_input,','))
            ...
            [t,is]=regexp(insertionsize_input,'[\[,\]"]','match','split');
            is=str2double(is(~cellfun(@isempty,is)));
...
aeb_script = fullfile(libdir,'align_each_bkpt5.pl'); 
aeb_binary = 0;
if ~exist(aeb_script,'file')
	aeb_script = fullfile(libdir,'align_each_bkpt5.bin');
	aeb_binary = 1;
	fprintf('Using binary script %s\n',aeb_script);
else
	fprintf('Using perl script %s\n',aeb_script);
end
demand_file(aeb_script);
...
prepare_breakpointer_temp_files3(drr,bam,firstidx,lastidx,P);
...
if aeb_binary
	system([aeb_script ' ' num2str(firstidx) ' ' num2str(lastidx) ' ' fusedname '.fasta ' fileid2 ' ' splitname ' ' libdir]);
else
