...
#main <- function(...) {
	...
	usage <- paste(sep="","\r\n", "Usage: createReport.R -LnozzlePath -CcaseListFile -Ffigurefile -TtableSummaryFile -MfilteredMafFile -RmafFile\r\n",
			...
			"\r\n",
			"$HeadURL: https://svn.broadinstitute.org/CancerGenomeAnalysis/trunk/matlab/seq/oxoGFilterv3NozzleReport/tags/v1.10.4.0/createReport.R $"
			);
	args = commandArgs();
	...
	oxoReport <- addToResults(oxoReport, newParagraph(
			paste(sep=" ", 
					...
					"<br>picard oxoQ -- per base sequence error rate attributable to oxo-G  false G>T sites. OxoQ units in phred quality score (-10log10(error)) for the given sample.  OxoQ values greater than ~35 are evidence that oxo-G errors do not result in higher error rates than other sources sequencing errors. This value can be used by the filter to suppress rejection of oxoG artifacts."
	...
	oxoqFilename = paste(sep="",  destFigureDir, '/', basename(filteredMafFile), "_oxoQ.png");
	...
	if (length(caseLines) > 1){		
		...
		figOxoQ <- newFigure(paste(sep="",  destFigureDir, '/', basename(filteredMafFile), "_oxoQ.png"), "Numbers of rejected (top) and passed (bottom) mutations vs. picard oxoQ value.  Blue (x) mark final filter results, red (+) mark filter results w/o OxoQ correctoins, and green (o) mark unfiltered mutation counts");
		oxoReport <- addToSummary(oxoReport, figOxoQ);
	...
	oxoReport <- addToMethods( oxoReport ,
			...
			newParagraph(paste(sep=" ","oxoQ_param1 = 36", "(oxoQ threshold for sigmoid function where filter effect is reduced by factor of 2)")),
			newParagraph(paste(sep=" ","oxoQ_param1 = 1.5", "(oxoQ sigmoid width parameter to contol how filter effect is reduced at high oxoQ)"))
