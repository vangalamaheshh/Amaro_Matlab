...
main <- function(...) {
	...
	usage <- paste(sep="","Usage: createReport.R -LnozzlePath -CcaseListFile -Ffigurefile -TtableSummaryFile -MfilteredMafFile -RmafFile\r\n",
	...
	if (length(caseLines) > 1){		
		...
		figO_A <- newFigure( paste(sep="",  destFigureDir, '/', basename(filteredMafFile), " All Artifact Mode_ArtifactMode_hist2d_FDR.png"), "All artifact mode mutations for all cases.  A patch (where mutations are filtered) cannot be displayed, since this varies from case to case.");
