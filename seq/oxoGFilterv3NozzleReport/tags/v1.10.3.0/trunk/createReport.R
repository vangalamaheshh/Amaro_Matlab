...
#main <- function(...) {
	...
	usage <- paste(sep="","\r\n", "Usage: createReport.R -LnozzlePath -CcaseListFile -Ffigurefile -TtableSummaryFile -MfilteredMafFile -RmafFile\r\n",
			...
			"\r\n",
			"$HeadURL: https://svn.broadinstitute.org/CancerGenomeAnalysis/trunk/matlab/seq/oxoGFilterv3NozzleReport/tags/v1.10.3.0/trunk/createReport.R $"
			);
	args = commandArgs();
	...
	if (length(caseLines) > 1){		
		...
		figOxoQ <- newFigure(oxoqFilename, "Numbers of rejected (top) and passed (bottom) mutations vs. picard oxoQ value.  Blue (x) mark final filter results, red (+) mark filter results w/o OxoQ correctoins, and green (o) mark unfiltered mutation counts");
