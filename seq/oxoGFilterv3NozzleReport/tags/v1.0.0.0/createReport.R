...
#main <- function(...) {
	...
	usage <- paste(sep="","Usage: createReport.R -CcaseListFile -Ffigurefile -TtableSummaryFile -MfilteredMafFile -RmafFile\r\n",
	...
	if (file.exists(tableSummaryFile) == TRUE) {
		...
		file.copy(tableSummaryFile, destTableSummaryFile);
		summaryTableObj = read.table(tableSummaryFile, header = TRUE);
	} else {
		stop(paste('Could not find file containing case list:', destTableSummaryFile, sep=" "));
	}
	...
	oxoReport <- addToSummary(oxoReport,
			newParagraph(paste(sep="", length(caseLines), " case(s) were processed in this run: ", "<ol>"))
			);
	for(i in 1:length(caseLines)){
		oxoReport <- addToSummary(oxoReport,
				newParagraph("<li>",caseLines[i], "</li>")
		);				
	}
	oxoReport <- addToSummary(oxoReport,
			newParagraph(paste(sep="","</ol>"))
	);		
	
	## RESULTS (incl. FIGURES)
	# Create and insert table
	summaryTable <- newTable(summaryTableObj, "Counts and estimates for all cases in the given maf file", file=destTableSummaryFile);
	...
	oxoReport <- addToResults(oxoReport, newParagraph(
			paste(sep=" ", 
					...
					"<br>PoxoG -- estimated PoxoG from this sample.  Note that this is not the value used in the filter and is included for informational purposes and possible use in future functionality.",
	...
	mafFileSubSection <- addTo(mafFileSubSection, newParagraph(paste(sep="","Filtered: ", asLink(basename(filteredMafFile)) )));
	mafFileSubSection <- addTo(mafFileSubSection, newParagraph(paste(sep="","Unfiltered: ", asLink(basename(mafFile)) )));
	...
	if (length(caseLines) == 1) {
		...
		for(i in 1:length(caseLines)){
			...
			#);
			caseFigureSubsection = newSubSection(paste("<h3>Case ", i ,": ", caseName,"</h3>"));
			caseFigureSubsection <- addTo( caseFigureSubsection, newParagraph(paste("<h3>Case ", i ,": ", caseName,"</h3>")) );
			### Create Figures
			...
			figure1 <- newFigure( orchestraPlot_A, 
					"Histogram of artifact mode mutations.  All mutations under the grey patch have been filtered." );
			caseFigureSubsection <- addTo( caseFigureSubsection , figure1);
			...
			caseFigureSubsection <- addTo( caseFigureSubsection , figure2);
			...
			caseFigureSubsection <- addTo( caseFigureSubsection , figure3);
			
			lego_after <-  paste(sep="", destFigureDir, '/', caseName,"_lego_after.png");
			figure4 <- newFigure( lego_after,
					"Mutation spectrum after filtering." );
			caseFigureSubsection <- addTo( caseFigureSubsection , figure4);
		
			figureSubSection <- addTo(figureSubSection, caseFigureSubsection);
		}
	...
	## Summary Figures
	figRR <- newFigure( paste(sep="",  destFigureDir, '/', basename(filteredMafFile), ".RR.png"), "Rejected real estimates for all cases");
	oxoReport <- addToSummary(oxoReport, figRR);
	figPA <- newFigure( paste(sep="",  destFigureDir, '/', basename(filteredMafFile), ".PA.png"), "Passed artifact estimates for all cases");
	...
	if (length(caseLines) > 1){		
		...
		figO_A <- newFigure( paste(sep="",  destFigureDir, '/', basename(filteredMafFile), " All Artifact Mode_ArtifactMode_hist2d_FDR.png"), "All artifact mode mutations for all cases");
		oxoReport <- addToSummary(oxoReport, figO_A);
		figO_nA <- newFigure( paste(sep="",  destFigureDir, '/', basename(filteredMafFile), " All Non-Artifact Mode_NonArtifactMode_hist2d.png"), "All non-artifact mode mutations for all cases");
	...
	writeReport( oxoReport , filename="report", level=PROTECTION.PUBLIC );	
