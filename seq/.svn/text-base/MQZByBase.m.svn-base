...
for s=1:length(samples)
  ...
  for i=1:2
    ...
    OutDir{s,i} = ['/xchip/tcga_scratch/lawrence/' sample '/' tn '_mqz'];
...
while(~all_done)
  ...
  for s=1:length(samples)
    ...
    for i=1:2
      ...
      for c=1:24
        file = [OutDir{s,i} '/chr' num2str(c) '.mqz'];
        if exist(file,'file')
          dcbb = dir(file);
        ...
        cmd = ['"java -Xmx2g -classpath ' java_classpath ' '...
           'MQZByBase ' BAMFile{s,i} ' ' BAIFile{s,i} ' ' ...
           file ' ' num2str(c) '"'];
...
if did_nothing, fprintf('All MQZ files are already up-to-date!\n'); end
