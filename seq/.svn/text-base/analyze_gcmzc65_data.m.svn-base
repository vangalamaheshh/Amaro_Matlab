function R = analyze_gcmzc65_data(Nn,outname)

Z = load_struct('/xchip/cga1/lawrence/db/gcmzc65/categs.txt');
tmp = parse(Z.name,'(.*):(.*):(.*):(.*):(.*)$',{'goodbad','cons','expr','zone','categ'});
...
tmp = stringsplice([Z.expr Z.cons Z.zone Z.base Z.context],1,':');
...
% calculate rates
y = {'conserved','nonconserved'}; ylabel=y;
...
R = []; R.categ=[]; R.cons=[]; R.zone=[]; R.context=[]; R.Ncov=[]; R.n_muts=[]; R.mutrate=[]; r=1;
R.stdev=[]; R.ci_low=[]; R.ci_high=[];

for qi=1:length(qlabel), idx1a = find(strcmp(q{qi},Z.expr));
 for yi=1:length(ylabel), idx1 = idx1a(strcmp(y{yi},Z.cons(idx1a)));
  for ci=1:length(clabel)
    ...
    for zi=1:length(zlabel), idx3 = idx2(grep(z{zi},Z.zone(idx2),1));
      R.categ{r,1} = qlabel{qi};  R.zone{r,1} = zlabel{zi};  R.cons{r,1} = ylabel{yi}; R.context{r,1} = clabel{ci};
