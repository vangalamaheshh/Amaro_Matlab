...
P = impose_default_value(P,'method','greedy');   %  "greedy" or "best"
P = impose_default_value(P,'max_k',5);
P = impose_default_value(P,'report_filename',[]);

if size(Nn,1)~=32, error('input must have 32 rows'); end
if size(Nn,2)~=5, error('input must have 5 columns (N A C G T)'); end

orig_N = [Nn(1:16,1)';Nn(17:32,1)'];
orig_n = [Nn(1:16,[3 4 5])';Nn(17:32,[2 4 5])'];

if ~isempty(P.report_filename)
  report_fh = fopen(P.report_filename,'wt');
end

% reformat n into 4x4x2x3
% reformat N into 4x4x2
...
for base5 = 1:4
  for base3 = 1:4
    for oldbase = 1:2
      ...
      N(base5,base3,oldbase) = orig_N(ceil(row/3),col);
end,end,end

Ntot = sum(N(:));
unsplit = {[1:4],[1:4],[1:2],[1:3]};

% make lookup table of entropies

fprintf('Making lookup table... \n');
mx = (2^16)-1;
lookup = nan(mx,1);
for i=1:mx, if ~mod(i,10000), fprintf('%d/%d ',i,mx); end
...
switch(lower(P.method))
  ...
  for sz=1:P.max_k
    ...
    subfprintf('%d possible category set', length(leaves));
    if length(leaves)>1,subfprintf('s');end
    subfprintf('\n');
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   for k=1:P.max_k
       ...
       subfprintf('before condensing: %d\n',lastleaf);
       leaves = unique(sort(leaves(1:lastleaf,:),2),'rows');
     end
     subfprintf('%d possible category set', size(leaves,1));
     if length(leaves)>1,subfprintf('s');end
     subfprintf('\n');
     ...
     best_l = 0; best_H = Inf;
     for l=1:length(leaves)
       if ~mod(l,100000), fprintf(' %d/%d ',l,length(leaves)); end
       H = 0;
       for i=1:k
...
% subfunctions
  ...
  function x = npartsum(cut)
    x = n(cut{1},cut{2},cut{3},cut{4});
    x = sum(x(:));
  end

  function x = Npartsum(cut)
    x = N(cut{1},cut{2},cut{3});
    x = sum(x(:));
  end
  ...
  function s = rulestats(parts)
    ...
    s.N = zeros(np,1);
    for p=1:np
      s.N(p) = Npartsum(parts{p});
      s.n(p) = npartsum(parts{p});
    end
    [s.rate s.ci] = binofit(s.n,s.N);  
  ...
  function stats = reportrule(parts)
    ...
    for j=1:length(parts)
      i=ord(j);
      subfprintf('  %-19s n %5.0f N %10.0f  rate %.2e (%sx)  ci %.2e to %.2e\n',...
              rules{i},stats.n(i),stats.N(i),stats.rate(i),...
              format_number(stats.relrate(i),3,4),stats.ci(i,1),stats.ci(i,2));
    end
  end

  function stats = reportrule2(partsi)
    for i=1:length(partsi)
      parts{i,1} = int_to_categ(partsi(i));
    end
    rules = convert_parts_to_rule(parts);
    stats = rulestats(parts);
    [tmp ord] = sort(stats.relrate,'descend');
  ...
  function LL = remove_duplicate_leaves(L)
    ...
    len = zeros(nl,1);
    for l=1:nl
      if ~mod(l,10000), fprintf(' %d/%d ',l,nl); end
      leaf = L{l};
      nc = length(leaf);
      ileaf = zeros(nc,1);
      for c=1:nc
