function X = dRanger_annotate_sitesXXX(X,P,R)
%
% replaces dRanger_annotate and annotate_site
%
...
P = impose_default_value(P,'build_refseq',[P.build '/R.mat']);
P = impose_default_value(P,'refdir',P.build);
...
nx = slength(X);
if ~isnumeric(X.chr1), X.chr1 = convert_chrXXX(X.chr1,P);end
...
if ~isnumeric(X.chr2), X.chr2 = convert_chrXXX(X.chr2,P);end
...
if ~exist('R','var'), R = load_refseqXXX(P.build,P.refdir); end
if iscell(R.chr), R.chr = convert_chrXXX(R.chr,P); end
