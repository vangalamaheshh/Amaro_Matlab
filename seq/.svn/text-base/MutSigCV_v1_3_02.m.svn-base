...
function MutSig_runCV(mutation_file,coverage_file,covariate_file,output_file)
  ...
  np = slength(pat);
  if np == 1, error('mutation_file must contain more than one patient'); end
  ...
  a=find(n_silent>N_silent);
  if ~isempty(a)
	fprintf('Warning: more silent mutations found than there is coverage.  Clipping them to the coverage value.\n')
    n_silent(a) = N_silent(a);
  end
  
  a=find(n_nonsilent>N_nonsilent);
  if ~isempty(a)
	fprintf('Warning: more nonsilent mutations found than there is coverage.  Clipping them to the coverage value.\n')
    n_nonsilent(a) = N_nonsilent(a);
  end

  a=find(n_noncoding>N_noncoding);
  if ~isempty(a)
	fprintf('Warning: more noncoding mutations found than there is coverage.  Clipping them to the coverage value.\n')
    n_noncoding(a) = N_noncoding(a);
  end
  ...
  if abs_log2_difference_nonsilent_silent>max_abs_log2_difference_nonsilent_silent, fprintf('Warning: silent and nonsilent rates are too different'); end
  ...
  [tmp,null_indel_categ] = max(sum(sum(N_nonsilent,1),3));       % null+indel categ has the highest coverage value
  N_silent(:,end+1,:) = N_silent(:,null_indel_categ,:);          % copy total coverage from null+indel coverage
  N_nonsilent(:,end+1,:) = N_nonsilent(:,null_indel_categ,:);
  N_noncoding(:,end+1,:) = N_noncoding(:,null_indel_categ,:);
  ...
  n_gcp_bg = n_silent + n_noncoding;
  N_gcp_bg = N_silent + N_noncoding;
  x_gcp2 = zeros(size(n_gcp_bg));  X_gcp2 = zeros(size(N_gcp_bg));
  ...
  for gi=1:ng, if ~mod(gi,1000), fprintf('%d/%d ',gi,ng); end
    ...
    for ni=0:max_neighbors, gidx = ord(ni+1);
      ...
      x_gcp2(gi,:,:) = x_gcp2(gi,:,:) + n_gcp_bg(gi,:,:);  X_gcp2(gi,:,:) = X_gcp2(gi,:,:) + N_gcp_bg(gi,:,:);
...
%   
%  % Swap in alternative calculation for bagels
%  x_gcp = x_gcp2; X_gcp = X_gcp2;
  ...
  for g=1:ng, if ~mod(g,1000), fprintf('%d/%d ',g,ng); end
    ...
    Sdeg(priority2==null_indel_categ) = Sdeg(priority2==null_indel_categ) + null_score_boost;
...
function p=hyge2pdf(k,n,k1,n1)
  if any(rem(k(:),1)~=0) || any(rem(n(:),1)~=0) || any(rem(k1(:),1)~=0) || any(rem(n1(:),1)~=0)
