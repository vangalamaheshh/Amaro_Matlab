function dRanger_draw_circos(sample,P)
...
if ~isempty(grep('chr',X.chr1)), X.chr1 = convert_chr(X.chr1); end
if ~isempty(grep('chr',X.chr2)), X.chr2 = convert_chr(X.chr2); end
...
if length(unique(k.chr))<21, error('Error processing karytype file'); end
k.chr = convert_chr(k.chr);
for i=1:slength(X)
