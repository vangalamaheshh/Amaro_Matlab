...
function MutSigCV(flanking_option,mutation_file,coverage_file,covariate_file,output_file)

if nargin~=5, error('usage: MutSigCV(flanking_option,mutation_file,coverage_file,covariate_file,output_file)'); end

if ~strcmp(flanking_option,'--useflanking') && ~strcmp(flanking_option,'--ignoreflanking')
    error('flanking_option must be --useflanking or --ignoreflanking');
end
...
% ensure all genes are in common
MC_incomplete = setxor(M.Hugo_Symbol,C.gene);
MG_incomplete = setxor(M.Hugo_Symbol,G.gene);
GC_incomplete = setxor(G.gene,C.gene);
incomplete_genes = unique([MC_incomplete;MG_incomplete;GC_incomplete]);
if ~isempty(incomplete_genes)
    fprintf('Warning... found %d genes that do not have information across all three inputs',length(incomplete_genes));
    %incomplete_genes
    fprintf('Removing these genes from consideration');
    M_rows_to_remove = ismember(M.Hugo_Symbol,incomplete_genes);
    M = reorder_struct(M,~M_rows_to_remove);
    C_rows_to_remove = ismember(C.gene,incomplete_genes);
    C = reorder_struct(C,~C_rows_to_remove);
    G_rows_to_remove = ismember(G.gene,incomplete_genes);
    G = reorder_struct(G,~G_rows_to_remove);
end
...
generic_column_name = 'x__GENERIC__';
if np == 1 && strcmp(patient_names{1},generic_column_name)
    patient_names = unique(M.patient);
    np = length(patient_names);
    for i=1:np
        C.(patient_names{i})=C.(generic_column_name);
    end
end
...
for ci=1:ncat
  silent_idx = strcmp(C.effect,'silent') & C.categ==ci;
  nonsilent_idx = strcmp(C.effect,'nonsilent') & C.categ==ci;
  flank_idx = strcmp(C.effect,'flank') & C.categ==ci;
  for pi=1:np
...
if strcmp(flanking_option,'--ignoreflanking')
    n_flank = zeros(size(n_flank));
    N_flank = zeros(size(N_flank));
elseif strcmp(flanking_option,'--useflanking')
    zero_flank_mut = ~any(n_flank(:));
    zero_flank_cov = ~any(N_flank(:));
    if zero_flank_cov && ~zero_flank_mut
        error('There are flanking mutations, but there is no flanking coverage.'); 
    end
