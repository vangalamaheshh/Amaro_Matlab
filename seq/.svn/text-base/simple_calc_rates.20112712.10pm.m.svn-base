...
k1 = 1.96;
k2 = 1.96;
rbagelflank_c = max(0,rbagelflank_c-k1*sdbagelflank_c);
rbagelsil_c = max(0,rbagelsil_c-k1*sdbagelsil_c);
rflank_c = max(0,rflank_c-k2*sdflank_c);
rsil_c = max(0,rsil_c-k2*sdsil_c);
rsf_c = max(0,rsf_c-k2*sdsf_c);
rsphere_c = max(0,rsphere_c-k1*sdsphere_c);
rnon_c = max(0,rnon_c-k2*sdnon_c);
...
pbagelflank_c = nan(ng,ncat);
pbagelsil_c = nan(ng,ncat);
pflank_c = nan(ng,ncat);
psil_c = nan(ng,ncat);
psf_c = nan(ng,ncat);
psphere_c = nan(ng,ncat);
pnon_c = nan(ng,ncat);
for c=1:ncat
    pbagelflank_c(:,c) = bintail(M.gene.nbagelflank_c(:,c),M.gene.Nbagelflank_c(:,c),M.rflank_c(c));
    pbagelsil_c(:,c) = bintail(M.gene.nbagelsil_c(:,c),M.gene.Nbagelsil_c(:,c),M.rsil_c(c));
    pflank_c(:,c) = bintail(M.gene.nflank_c(:,c),M.gene.Nflank_c(:,c),M.rflank_c(c));
    psil_c(:,c) = bintail(M.gene.nsil_c(:,c),M.gene.Nsil_c(:,c),M.rsil_c(c));
    psf_c(:,c) = bintail(M.gene.nsf_c(:,c),M.gene.Nsf_c(:,c),M.rsf_c(c));
    psphere_c(:,c) = bintail(M.gene.nsphere_c(:,c),M.gene.Nsphere_c(:,c),M.rsf_c(c));
    pnon_c(:,c) = bintail(M.gene.nnon_c(:,c),M.gene.Nnon_c(:,c),M.rnon_c(c));
end

M.gene.s_c = repmat({'---------------------'},ng,1);

sbagelflank_c = -log10(pbagelflank_c);
sbagelsil_c = -log10(pbagelsil_c);
sflank_c = -log10(pflank_c);
ssil_c = -log10(psil_c);
ssf_c = -log10(psf_c);
ssphere_c = -log10(psphere_c);
snon_c = -log10(pnon_c);

M.gene.sbagelflank_c = round(sbagelflank_c);
M.gene.sbagelsil_c = round(sbagelsil_c);
M.gene.sflank_c = round(sflank_c);
M.gene.ssil_c = round(ssil_c);
M.gene.ssf_c = round(ssf_c);
M.gene.ssphere_c = round(ssphere_c);
M.gene.snon_c = round(snon_c);

M.gene.x2_c = repmat({'---------------------'},ng,1);

%%%%%%%%%%%%%%%%%%%%% DOWNWEIGHT UNCERTAIN F's

k = ones(ng,ncat); s = sbagelflank_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xbagelflank_c = max(0,fbagelflank_c.* k);

k = ones(ng,ncat); s = sbagelsil_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xbagelsil_c = max(0,fbagelsil_c.* k);

k = ones(ng,ncat); s = sflank_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xflank_c = max(0,fflank_c.* k);

k = ones(ng,ncat); s = ssil_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xsil_c = max(0,fsil_c.* k);

k = ones(ng,ncat); s = ssphere_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xsf_c = max(0,fsf_c.* k);

k = ones(ng,ncat); s = ssphere_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xsphere_c = max(0,fsphere_c.* k);

k = ones(ng,ncat); s = snon_c;
%k(s<4) = 0.5; k(s<3) = 0.2; k(s<2) = 0.1; k(s<1) = 0.05;
xnon_c = max(0,fnon_c.* k);

M.gene.xbagelflank_c = round(xbagelflank_c);
M.gene.xbagelsil_c = round(xbagelsil_c);
M.gene.xflank_c = round(xflank_c);
M.gene.xsil_c = round(xsil_c);
M.gene.xsf_c = round(xsf_c);
M.gene.xsphere_c = round(xsphere_c);
M.gene.xnon_c = round(xnon_c);


M.gene.xx_c = repmat({'---------------------'},ng,1);

%%%%%%%%%%% FINAL SCORING

method = 2;

bkgd = [xbagelflank_c xbagelsil_c xflank_c xsil_c xsf_c xsphere_c];
signal = [xnon_c];
