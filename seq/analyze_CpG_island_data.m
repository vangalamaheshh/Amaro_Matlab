function R = analyze_CpG_island_data(Nn,categdir,outstem,P)

if ~exist('P','var'), P=[]; end
P=impose_default_value(P,'output_format',2);
P=impose_default_value(P,'use_melanoma_categories',false);

Z = load_struct(['/xchip/cga1/lawrence/db/' categdir '/categs.txt']);
if size(Nn,1)~=slength(Z), error('size(Nn,1)~=slength(Z)'); end
Z.Nn = Nn; clear Nn;

% remove "bad" regions, noncoding regions, non-exons, and Ns
idx = grepv('any N|bad',Z.name,1);
Z = reorder_struct(Z,idx);

% parse names
tmp = parse(Z.name,'^(good|bad):(.*:)?([ACGT]) in ([^:]*):?.*$',...
  {'goodbad','other','base','context'});
Z = merge_structs({Z,tmp});
tmp = parse(Z.other,'(sea|shore|island).*(IGR|UTR|exon|intron)',{'cpgzone','zone'});
Z = merge_structs({Z,tmp});
...
z = Z.Nn(idx,5:-1:2); Z.Nn(idx,2:5) = z;
tmp = stringsplice([Z.zone Z.cpgzone Z.base Z.context],1,':');
[u ui uj] = unique(tmp,'first');
out = zeros(length(u),5);
for i=1:length(uj), out(uj(i),:)=out(uj(i),:)+Z.Nn(i,:); end
Z = reorder_struct(Z,ui);
Z.Nn = out;

% compute rates

qlabel = {'island','shore','sea','total'}; q = {'island','shore','sea','island|shore|sea'};
...
for qi=1:length(qlabel), idx1 = grep(q{qi},Z.cpgzone,1);
  for ci=1:length(clabel)
    ...
    for zi=1:length(zlabel), idx3 = idx2(grep(z{zi},Z.zone(idx2),1));
      ...
      R.Ncov(r,1) = sum(Z.Nn(idx3,1));  R.n_muts(r,1) = fullsum(Z.Nn(idx3,cols));
