function M = impute_callschemes(M)
...
M.patient.nmut = histc(M.mut.pat_idx,1:slength(M.patient));
M.patient.n_coding = histc(M.mut.pat_idx(M.mut.is_coding),1:slength(M.patient));
M.patient.n_flank = histc(M.mut.pat_idx(M.mut.is_flank),1:slength(M.patient));
M.patient.n_coding_nonsilent = histc(M.mut.pat_idx(M.mut.is_coding & ~M.mut.is_silent),1:slength(M.patient));
