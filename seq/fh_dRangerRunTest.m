function fh_dRangerRunTest(individual,tdir,ndir,tminmapq,minpairs,windowsize,nminwindow,normpaneldb,minsomratio,nminspanfrac,minscoreforbp,min_ignore_matchingnormal,build,refdir)
% fh_dRangerRun(individual,tdir,ndir,tminmapq,minpairs,windowsize,nminwindow,normpaneldb,minsomratio,nminspanfrac,minscoreforbp,build,refdir)
%
% Mike Lawrence 2010 
% Chip Stewart  2012 .. added refdir argument for canFam2 and other reference genomes

if nargin<3, error(['Usage: fh_dRangerRun(individual,tdir,ndir,tminmapq,minpairs,windowsize,'...
    'nminwindow,normpaneldb,minsomratio,nminspanfrac,minscoreforbp,min_ignore_matchingnormal,build,refdir)']); end
...
if ~exist('refdir','var'), refdir = ['/xchip/cga1/annotation/db/ucsc/' build]; end
...
P = impose_default_value(P,'refdir',refdir);
%P = impose_default_value(P,'build_refseq',[refdir '/R.mat']);
...
%  end
  ...
  % RUN CORE ALGORITHM
  [X TT.ridx NN.ridx] = dRanger_core_algorithmXXX(T,N,P);
  ...
  X = dRanger_annotate_sitesXXX(X,P);
  
  % SCORING
  
  % extract nuwp and fmapqz
  fprintf('Adding filtering metrics from *.fmapqz and *.nuwp files.\n');
  X = dRanger_add_filtering_from_boom_part2XXX(X,tdir,ndir,struct('add_coverage',false,'add_avgnmm',false));
