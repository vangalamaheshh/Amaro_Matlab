...
function MutSigCV(mutation_file,coverage_file,covariate_file,output_filestem,varargin)
  ...
  no_preprocess = (length(varargin)==1 & ischar(varargin{1}) & strcmpi(varargin{1},'-nopreprocess'));
  if ~no_preprocess
    % (1) PREPROCESS
    fprintf('\n\n');
...
function MutSig_preprocess(mutation_file,coverage_file,covariate_file,output_filestem,mutation_type_dictionary_file,chr_files_directory,categ_flag)
  ...
  if ischar(categ_flag)  % convert string number if necessary; else it might be a path to use to a categs.txt
    tmp = str2double(categ_flag);
    if ~isnan(tmp), categ_flag = tmp; end
  end
  ...
  if isfield(M,'is_coding') || isfield(M,'is_silent')
    ...
    fprintf('       Requires type/Variant_Classification column and mutation_type_dictionary so we can assign nulls.\n');
  ...
  if isfield(M,'effect')
    ...
    % try to calculate EFFECT column from type/Variant Classification
    if isfield(M,'Variant_Classification')
      fieldname = 'Variant_Classification';
    elseif isfield(M,'type')
      fieldname = 'type';
    else
      error('mutation_file is missing type/Variant_Classification');
    end
    ...
    M.effect = mapacross(upper(M.(fieldname)),upper(dict.Variant_Classification),dict.effect,'unknown');
  ...
  if ischar(categ_flag)
    if exist(categ_flag,'file')
      categ_txt_bypass_file = categ_flag;
      method = 3;
    else
      categ_flag = nan;
    end
  end
  if isnumeric(categ_flag)
    if categ_flag==0
  ...
  elseif method==2
    ...
    % assign categories
    M.categ_ignoring_null_categ = repmat(K.name(1),slength(M),1);
  ...
  elseif method==3
    ...
    fprintf('Preparing for category collapsing...\n');
    ...
    % STEP3
    % Category Discovery
    if exist('categ_txt_bypass_file','var')
      fprintf('Loading preset categories from %s\n',categ_txt_bypass_file);
      K = load_struct(categ_txt_bypass_file);
      K = reorder_struct_exclude(K,grepmi('indel|null',K.name));
    else
      fprintf('Discovering best set of %d categories...\n',ncategs);
    ...
    X.kidx = nan(slength(X),1); for i=1:slength(X), X.kidx(i) = find(c(X.context65(i),:,X.newbase_idx(i))); end
    ...
    M.categ_ignoring_null_categ = M.categ;
    ...
    if isfield(K,'N') && isfield(K,'n') && isfield(K,'rate') && isfield(K,'relrate')
  ...
  % (1) mutation file
  M = rmfield_if_exist(M,{'chr_idx','triplet','yname','newbase_idx','context65_right','triplet_middle'});
...
function MutSig_runCV(mutation_file,coverage_file,covariate_file,output_file)
  ...
  % additional MAF requirements for permutations
  demand_field(M,{'chr','start','newbase'});  % (need to add correspondences to Chromosome, Start_position)
  M.chr = convert_chr(M.chr);
  M = make_numeric(M,'start');
  tmp = regexprep(M.newbase,'^(.).*$','$1');
  M.newbase_idx = listmap(tmp, {'A', 'C', 'G', 'T'});
  M.newbase_idx(M.effect_idx==listmap('indel_coding',effect_names))=5;   % 1=A 2=C 3=G 4=T 5=indel
  ...
  if isfield(M,'effect')
    M.effect = regexprep(M.effect,'^flank.*','noncoding');
    midx = find(grepmi('ins|del',M.type) & ~strcmp('noncoding',M.effect));
    M.effect(midx) = repmat({'indel_coding'},length(midx),1);
  ...
  if ~isempty(bad)
    ...
    if isfield(M,'ref_allele') && isfield(M,'newbase') && isfield(M,'type')
      ...
      bad2 = bad(is_probably_indel&is_probably_noncoding);
      if ~isempty(bad2)
  ...
  if all(~isnan(knum))
    ...
    % make sure all coding indels are assigned to this category (especially for the ignoring_null_categ index)
