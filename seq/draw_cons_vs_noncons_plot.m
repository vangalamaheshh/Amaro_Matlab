function draw_cons_vs_noncons_plot(R,P)

if ~exist('P','var'), P=[]; end
P = impose_default_value(P,'cons_term','cons');  % can set to "RP" if needed
...
barweb(reshape(Rtot.mutrate,3,3)',reshape(Rtot.ci_low,3,3)',reshape(Rtot.ci_high,3,3)',0.8,{P.cons_term,['non' P.cons_term],'total'});
...
barweb(reshape(Rc.mutrate,3,3)',reshape(Rc.ci_low,3,3)',reshape(Rc.ci_high,3,3)',0.8,{P.cons_term,['non' P.cons_term],'total'});
...
barweb(reshape(Rnc.mutrate,3,3)',reshape(Rnc.ci_low,3,3)',reshape(Rnc.ci_high,3,3)',0.8,{P.cons_term,['non' P.cons_term],'total'});
