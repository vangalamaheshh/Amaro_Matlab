function fig = mutfig(inputfile,outputfile,param)
...
for i=1:nf
  if isempty(F.color(i))
    ...
    elseif strcmpi('mutation', F.feature{i})
      if strncmpi('Missense Mutation', F.type{i},8)
        F.color{i} = [0 0.5 0.5];
      elseif strncmpi('indel', F.type{i},6) || strncmpi('Frame Shift Ins', F.type{i},6) || strncmpi('Frame Shift Del', F.type{i},6)
        F.color{i} = [1 0.6 0];
      elseif strncmpi('Splice Site', F.type{i},6)
        F.color{i} = [0.6 0 0.6];
      elseif strncmpi('Nonsense Mutation', F.type{i},8)
        F.color{i} = [1 0 0];
      elseif strncmpi('Silent',F.type{i},6)
          F.color{i}=[.5 .5 .5];
    ...
    elseif strcmpi('domain', F.feature{i})
      % choose random subdued color for this domain type
      idx = grep(['^' F.type{i} '$'],F.type,1);
      tries=0;
      done=false;
      while(~done && tries<1000)
...
if P.mutfig_style == 1
  ...
  M2=reorder_struct(M2,~isnan(M2.end));
  M2=reorder_struct(M2,~ismember(M2.label,''));
  for x=1:length(ui)
...
D = reorder_struct(D, ~isnan(D.start));
for i=1:length(D.label)
  ...
  y = fig.prot.bottom;
  h = fig.prot.height;
rectangle('position',[x1 y w h], 'FaceColor', D.color(i,:),'edgecolor',D.color(i,:));
end
...
%   M.color_sat(i) = M.color_hsv(i);
% end
% M = sort_struct(M,'color_sat',1);  % sort in forward order so that unique() picks the most-saturated (last) example

[tmp ui uj] = unique(M.type);
M3 = reorder_struct(M,sort(ui));
M3.type = regexprep(M3.type,'_','\_');   % protect underscores
y = fig.legend.y;
for i=1:length(M3.type)
  sx = x;
  sy = y;
  color = M3.color(i,:);
  if shape_legend
...
tightfig
set(gcf,'PaperSize',[22 42])
[names_l values_l]=count(M.label);
fig.maxy = fig.maxy + (max(names_l)*0.035);
fig.miny = ymin-0.035;
 set(gcf, 'position',[fig.pos.left fig.pos.bottom fig.width fig.height + 0.15]);
  set(gca,'YLim',[fig.miny fig.maxy]);
