function tab=load_table(fname,headline,dlm,headlines, getcols, maxlines, varargin)
...
tab.dat=textscan(fid,format,maxlines,'HeaderLines',headlines,'Delimiter',dlm,'bufsize',4096*8-1,'CommentStyle',headline{end}); % {tab.header{1},'','%',''}); %,varargin{:});
fclose(fid);
...
for i=1:NV
    ...
    if (~abc)
        % date with two '-' separating year month day
        abc=any(cellfun(@length,strfind(v1,'-'))>1);
    end
...
tline = fgetl(fid);
while ischar(tline)
    if max(isequal(commentchars{1},tline(1:2))|ismember(commentchars{2},tline(1)))==0
        fclose(fid);
        return
    end
