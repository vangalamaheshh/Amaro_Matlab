...
%   $Revision: 1.2.4.3 $  $Date: 2004/08/20 20:05:59 $

if nargin<1
    error('stats:mvnpdf:TooFewInputs','Input argument X is undefined.');
elseif ndims(X)>2 || isempty(X)
    error('stats:mvnpdf:InvalidData','X must be a non-empty matrix.');
end
