...
%'/xchip/cga_home/amaro/CLL/ABSOLUTE/AggregatedCCFCLL8.called.3.18.seg'
...
%BLACKLISTING:
SEG.ID(ismember(SEG.ID,'tri12')&ismember(SEG.individual,'GCLL-0042'))={'NaN'};
SEG.ID(ismember(SEG.individual,'GCLL-0291')&ismember(SEG.ID,'del11q'))={'NaN'};
%WHITELISTING:
SEG.ID(find((SEG.xStart==2131422930)&ismember(SEG.individual,'GCLL-0313')))={'del13q'};
SEG.ID(find(ismember(SEG.sample,'CLL-GCLL-0204-TR-NT-SM-41JOS-SM-4DOIQ')&SEG.xStart==1925915653))={'del11q'};
    ...
    for j=1:length(individuals)
        if ~isnan(SCNA_table.(strcat(events{i},'TP1'))(j))
            ...
            elseif SCNA_table.(strcat(events{i},'TP1_Low'))(j)>SCNA_table.(strcat(events{i},'TP2_High'))(j) %% Blue
                ...
                num_unchange=num_unchange+1;
            end
            count=count+1;
        end
    end
    p.shift(i,1)=1-binocdf(num_up+num_down,num_up+num_down+num_unchange,.5);
    p.stable(i,1)=1-binocdf(num_unchange,num_up+num_down+num_unchange,.5);
    p.event{i,1}=events{i};
    p.num_up(i,1)=num_up;
    p.num_down(i,1)=num_down;
    p.num_stable(i,1)=num_unchange;
    p.total(i,1)=num_up+num_down+num_unchange;
    %p(i)=1-binocdf(num_up,num_up+num_down,.5);
