...
function ex_seg=SCNA_CCF_chi2(A,sample_name,ex_seg)
ex_seg=rmfield_if_exist(ex_seg,'header');
ex_seg=rmfield_if_exist(ex_seg,'headline');
ex_seg.sample=repmat(sample_name,slength(ex_seg),1);
...
fudge_factor=.1;
ex_seg.sigma_tau(isnan(ex_seg.sigma_tau))=0;
ex_seg.sigma_major=sqrt(ex_seg.sigma_major.^2+fudge_factor^2);
ex_seg.sigma_minor=sqrt(ex_seg.sigma_minor.^2+fudge_factor^2);
ex_seg.sigma_tau=sqrt(ex_seg.sigma_tau.^2+fudge_factor^2);
...
A=load_table('~/Projects/CLL/ABSOLUTE/FullSetABSOLUTE.Table.CLL8.txt'); 
A=rmfield(A,'header');
A=rmfield(A,'headline');





for i=541:slength(SIF)
ex_seg=load_table(SIF.alleliccapseg_tsv{i});
out_seg=SCNA_CCF_chi2(A,SIF.pair_id(i),ex_seg);
save_struct(out_seg,strcat('/xchip/cga_home/amaro/CLL/ABSOLUTE/SCNA_CCF_recall/',SIF.pair_id{i},'CCF.seg'))
SIF.CCF_seg{i,1}=strcat('/xchip/cga_home/amaro/CLL/ABSOLUTE/SCNA_CCF_recall/',SIF.pair_id{i},'CCF.seg');
i  
end
