...
save(['~/Projects/Cancer/mouse/Gistic2/' SET '.SCNA.mat'],'-struct','XSCN')
save(['~/Projects/Cancer/mouse/Gistic2/' SET '.SCNA.structs.mat'],'XSCN','VS','VA')

%%

MM9=load_table('/Users/stewart/Projects/Cancer/mouse/ucsc.mm9/cytoBand.txt')
Nc=length(MM9.cytoband)
MM9.arm=cellstr(cellfun(@(x) x{1},regexp(MM9.cytoband,'[p-q]','match'),'UniformOutput', false))
MM9.arm=strcat(regexprep(MM9.chrn,'chr',''),'p')
MM9A=[];
a=unique(MM9.arm);
for i=1:length(a)
...
printStruct(MM9A,[],'~/Projects/Cancer/mouse/ucsc.mm9/arms.txt')
...
load(['~/Projects/Cancer/mouse/Gistic2/' SET '.SCNA.structs.mat'])
...
f=['~/Projects/Cancer/mouse/ucsc.mm9/arms.txt']
MM9A=load_table(f)
f='/Users/stewart/Projects/Cancer/mouse/purity/PR_MOUSE_SCLC_PAIR_Capture_All_Pairs.purity.25Oct2012.tsv'
P=load_table(f)
P.sample_id=cellstr(cellfun(@(x) [x{1} '-' x{2} '-Tumor-' x{5} '-' x{6}],regexp(P.pair,'-','split'),'UniformOutput', false))
addpath('/Users/stewart/CancerGenomeAnalysis/trunk/matlab/snp')
TX=load_table('~/Projects/Cancer/mouse/FH/mouse_mutation_summary_dgm.txt')
if (DATA==1)
...
elseif (DATA==5)
    Z=load_table('~/Projects/Cancer/mouse/FH/An_MOUSE_SCLC_PAIR.all.samples.txt')
    Z=trimStruct(Z,find(cellfun(@length,strfind(Z.sample_id,'-Tumor'))))
    Z.sample=regexprep(Z.sample_id,'-Tumor','')
    Z.individual_id=regexprep(Z.individual_id,'_','-')
